...
export default function MoviesArticle({ tvshowItem, videoSources = [] }) {
  ...
  useEffect(() => {
    if (playerReady && tvshowItem.source && tvshowItem.source.length === 11) {
      ...
      playerRef.current = new window.YT.Player("youtube-player", {
        ...
        playerVars: {
          ...
          modestbranding: 1,
          loop: 1,
          playlist: tvshowItem.source, // Repeat the same video for looping
        },
        events: {
          onReady: (event) => {
  ...
  return (
    <>
      <Head>
        <title>Movies Free™ – {tvshowItem?.title || "Default Title"}</title>
        ...
        <meta
          property="og:title"
          content=" Movies Free™ – Stay Informed, Stay Ahead "
        />
